...
function finley_register_author_profile_widget()
{
    register_widget('Graphthemes_Author_Profile');
}
add_action('widgets_init', 'finley_register_author_profile_widget');
...
if (!class_exists('Graphthemes_Author_Profile')) :
    ...
    class Graphthemes_Author_Profile extends WP_Widget
    {
        ...
        {
            parent::__construct(
                'finley_widget_author_profile', // Base ID
                __('Graphthemes: Author Profile', 'finley'), // Name
                array('description' => __('An Author Profile Widget', 'finley'),) // Args
            );
        }
        ...
        public function widget($args, $instance)
        {
            ...
            if (!filter_var($image, FILTER_VALIDATE_URL) === false) {
                $attachment_id = $obj->finley_widget_get_attachment_id($image);
            }
...
?>
            ...
            <div class="graphthemes-widget-author-bio-holder">
                ...
                <div class="text-holder">
                    ...
                    <?php if ($link && $label) { ?>
                        <a <?php if (isset($instance['target']) && $instance['target'] == '1') {
                                echo "target=_blank";
                            } ?> href="<?php echo esc_url($link); ?>" class="btn-readmore"><?php echo esc_html($label); ?></a>
                    <?php } ?>
        ...
        <?php
            $html = ob_get_clean();
            echo apply_filters('finley_widget_author_profile_widget_filter', $html, $args, $instance);
        ...
        ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title', 'finley'); ?></label>
                <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('name')); ?>"><?php esc_html_e('Author Name', 'finley'); ?></label>
                <input class="widefat" id="<?php echo esc_attr($this->get_field_id('name')); ?>" name="<?php echo esc_attr($this->get_field_name('name')); ?>" type="text" value="<?php echo esc_attr($name); ?>" />
            </p>

            <p>
                <label><?php esc_html_e('Display photo from:', 'finley'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('author-image-option' . '-gravatar')); ?>" class="radio-btn-wrap"><?php esc_html_e('Gravatar', 'finley'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('author-image-option' . '-photo')); ?>" class="radio-btn-wrap"><?php esc_html_e('Uploaded Photo', 'finley'); ?></label>
            </p>
            ...
            <p>
                ...
                <button class="upload_image_button button button-primary"><?php esc_html_e("Select Image", 'finley'); ?></button>
            </p>

            <p class="author-email">
                <label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php esc_html_e('Author Email', 'finley'); ?></label>
                <input class="widefat" id="<?php echo esc_attr($this->get_field_id('email')); ?>" name="<?php echo esc_attr($this->get_field_name('email')); ?>" type="text" value="<?php echo esc_attr($email); ?>" />
            </p>
            <div class="widget-side-note" class="example-text"><?php $grav_link = '<a href=' . esc_url( get_avatar_url( $email ) ) . ' target="_blank">' . esc_html__("Gravatar", 'finley') . '</a>';
                                                                echo sprintf(__('You can show your %1$s image instead of manually uploading your photo. Just add your gravatar registered email address here.', 'finley'), $grav_link); ?></div>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('content')); ?>"><?php esc_html_e('Description', 'finley'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('label')); ?>"><?php esc_html_e('Button Label', 'finley'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php esc_html_e('Button Link', 'finley'); ?></label>
            ...
            <p>
                <input class="widefat" id="<?php echo esc_attr($this->get_field_id('target')); ?>" <?php $j = '0';
                                                                                                    if (isset($instance['target'])) {
                                                                                                        $j = '1';
                                                                                                    } ?> value="1" <?php checked($j, true); ?> name="<?php echo esc_attr($this->get_field_name('target')); ?>" type="checkbox" />
                <label for="<?php echo esc_attr($this->get_field_id('target')); ?>"><?php esc_html_e('Open in New Tab', 'finley'); ?></label>
            </p>


            <div class="widget-social-profile">
                <label>
                    <h3><?php esc_html_e("Social Profile", 'finley'); ?></h3>
                </label>

                <label for="<?php echo esc_attr($this->get_field_id('social-facebook')); ?>"><?php esc_html_e('Facebook Profile', 'finley'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('social-linkedin')); ?>"><?php esc_html_e('LinkedIn Profile', 'finley'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('social-twitter')); ?>"><?php esc_html_e('Twitter Profile', 'finley'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('social-instagram')); ?>"><?php esc_html_e('Instagram Profile', 'finley'); ?></label>
