...
function finley_register_popular_post_widget()
{
    register_widget('Graphthemes_Widget_Popular_Post');
}
add_action('widgets_init', 'finley_register_popular_post_widget');
...
if (!class_exists('Graphthemes_Widget_Popular_Post')) :
    ...
    class Graphthemes_Widget_Popular_Post extends WP_Widget
    {
        ...
        {
            if (!is_customize_preview()) add_action('wp', array($this, 'finley_set_views'));

            parent::__construct(
                'finley_widget_popular_post', // Base ID
                esc_html__('Graphthemes: Popular Post', 'finley'), // Name
                array('description' => esc_html__('A Popular Post Widget', 'finley'),) // Args
            );
        }
         ...
         */
        function finley_set_views($post_id)
        {
            if (in_the_loop()) {
                $post_id = get_the_ID();
            } else {
         ...
         */
        function finley_get_views($post_id)
        {
            $count_key = '_finley_view_count';
            $count = get_post_meta($post_id, $count_key, true);
            if ($count == '') {
                return __("0 View", 'finley');
            } elseif ($count <= 1) {
                return $count . __(' View', 'finley');
            } else {
                return $count . __(' Views', 'finley');
            }
        }
        ...
        public function widget($args, $instance)
        {
            ...
            $title          = !empty($instance['title']) ?  $instance['title'] : __('Popular Posts', 'finley');
            ...
            if ($based_on == 'views') {
                ...
                $arg['meta_key'] = '_finley_view_count';
            ...
            $finley_widget_popular_post_size = apply_filters('finley_widget_popular_post_size', 'thumbnail');
...
?>
                        ...
                        <li>
                            ...
                            if ($show_thumbnail) { ?>
                                <a <?php echo $target; ?> href="<?php the_permalink(); ?>" class="post-thumbnail">
                                    ...
                                    if (has_post_thumbnail() && $show_thumbnail) {
                                        the_post_thumbnail($finley_widget_popular_post_size, array('itemprop' => 'image'));
                                    }
                                    ?>
                                </a>
                            <?php }
                            ...
                            <div class="entry-header">
                                ...
                                <?php if ($based_on == 'views' && $view_count) { ?>
                                    <span class="view-count"><?php echo esc_html($this->finley_get_views(get_the_ID())); ?></span>
            ...
            <?php
                $html = ob_get_clean();
                echo apply_filters('finley_widget_popular_post_widget_filter', $html, $args, $instance);
        ...
        public function form($instance)
        {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('num_post')); ?>"><?php esc_html_e('Number of Posts', 'finley'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('based_on')); ?>"><?php esc_html_e('Popular based on:', 'finley'); ?></label>
                <select id="<?php echo esc_attr($this->get_field_id('based_on')); ?>" name="<?php echo esc_attr($this->get_field_name('based_on')); ?>" class="based-on">
                    <option value="views" <?php selected($based_on, 'views'); ?>><?php esc_html_e('Post Views', 'finley'); ?></option>
                    <option value="comments" <?php selected($based_on, 'comments'); ?>><?php esc_html_e('Comment Count', 'finley'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('show_thumbnail')); ?>"><?php esc_html_e('Show Post Thumbnail', 'finley'); ?></label>
            </p>
            ...
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('show_postdate')); ?>"><?php esc_html_e('Show Post Date', 'finley'); ?></label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('target')); ?>">
                    <input class="widefat" id="<?php echo esc_attr($this->get_field_id('target')); ?>" name="<?php echo esc_attr($this->get_field_name('target')); ?>" type="checkbox" value="1" <?php echo checked($target, 1); ?> /><?php esc_html_e('Open in New Tab', 'finley'); ?> </label>
            </p>
...
<?php
        ...
        public function update($new_instance, $old_instance)
        {
            ...
            $instance = array();
            $instance['title']          = !empty($new_instance['title']) ? sanitize_text_field($new_instance['title']) : __('Recent Posts', 'finley');
            ...
            $instance['based_on']       = !empty($new_instance['based_on']) ? sanitize_text_field($new_instance['based_on']) : 'views';
            ...
            $instance['style']          = !empty($new_instance['style']) ? wp_kses_post($new_instance['style']) : 'style-one';
            $instance['target']         = !empty($new_instance['target']) ? wp_kses_post($new_instance['target']) : '';
