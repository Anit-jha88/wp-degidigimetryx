                    ...
                    if (count($menu_lists) > 0) {
                        foreach ($menu_lists as $menu):
                            ...
                            <tr>
                                ...
                                <td><a href="<?php echo admin_url('admin.php'); ?>?page=tgdprcl-manage-cookie-settings&action=edit&id=<?php echo $menu->id; ?>"><div class="wcui_cookie_bar_name"><?php echo esc_attr($menu->name); ?></div></a></td>
                                ...
                                <td><?php echo!empty($display_settings['layout']['display_type']) ? (($display_settings['layout']['display_type'] == 'bar') ? ucwords(esc_attr($display_settings['layout']['bar_position'])) : (($display_settings['layout']['display_type'] == 'popup') ? ucwords(esc_attr($display_settings['layout']['popup_position'])) : (($display_settings['layout']['floating_position']) ? ucwords(esc_attr($display_settings['layout']['floating_position'])) : ''))) : esc_attr__('Error', TGDPRCL_DOMAIN); ?></td>

                                <td><?php echo esc_attr(($display_settings['layout']['display_type'] == 'bar') ? ($tgdprcl_template_key_value_pair[$display_settings['layout']['bar_template_type']]) : (($display_settings['layout']['display_type'] == 'popup') ? ($tgdprcl_template_key_value_pair[$display_settings['layout']['popup_template_type']]) : (($display_settings['layout']['display_type'] == 'floating') ? ($tgdprcl_template_key_value_pair[$display_settings['layout']['floating_template_type']]) : ''))); ?></td>

                                <td><div class="cookie_settings_options">
                                        <a href="<?php echo admin_url('admin.php'); ?>?page=tgdprcl-manage-cookie-settings&action=edit&id=<?php echo intval($menu->id); ?>" class="wpcui_edit_button"></a>

                                        <a href="<?php echo get_home_url(); ?>?tgdprc_cookie_preview=<?php echo intval($menu->id) ?>" target="_blank" class="wpcui_preview_button"></a>
                                        <?php
                                        $nonce1 = wp_create_nonce('delete_setting_nonce');
                                        ?>
                                        <a href="<?php echo admin_url('admin-post.php'); ?>?action=delete_choosen_setting&id=<?php echo $menu->id; ?>&_wpnonce=<?php echo esc_attr($nonce1); ?>" onclick="return confirm('Do you want to delete?')" class="tgdprc_delete_button"></a>
                                        <?php
                                        $nonce2 = wp_create_nonce('copy_setting_nonce');
                                        ?>
                                        <a href="<?php echo admin_url('admin-post.php'); ?>?action=copy_choosen_setting&id=<?php echo $menu->id; ?>&_wpnonce=<?php echo esc_attr($nonce2); ?>" onclick="return confirm('Do you wish to copy?')" class="tgdprc_copy_button"></a>
                                    </div></td>
